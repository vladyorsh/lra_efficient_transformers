...
def model_postprocess(model):
  model.blocks = nn.ModuleList([ SelfLunaBlock(BPE_CLS_SETUP['hidden_dim'], BPE_CLS_SETUP['qkv_dim'], BPE_CLS_SETUP['mlp_dim'], BPE_CLS_SETUP['num_heads'], BPE_CLS_SETUP['internal_dropout_rate'], BPE_CLS_SETUP['affine']).to(BPE_CLS_SETUP['device']) for _ in range(BPE_CLS_SETUP['num_blocks']) ])
